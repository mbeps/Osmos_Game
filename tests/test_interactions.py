...
def test_hit_ball_detects_collision(interaction_factory):
    interaction = interaction_factory()
    ball1 = Ball(Vector(0, 0), Vector(0, 0), 5)
    ball2 = Ball(Vector(9, 0), Vector(0, 0), 5)
...
def test_engulf_updates_enemy_list_and_score(interaction_factory):
    player = Player(Vector(50, 50), Vector(0, 0), 12)
    enemy = Enemy(Vector(60, 50), Vector(0, 0), 5)
    interaction = interaction_factory(player=player, enemies=[enemy])
...
def test_engulf_when_player_is_smaller_marks_player_dead(interaction_factory):
    player = Player(Vector(10, 10), Vector(0, 0), 5)
    enemy = Enemy(Vector(12, 10), Vector(0, 0), 10)
    interaction = interaction_factory(player=player, enemies=[enemy])
...
def test_bounce_reflects_velocity_against_wall(interaction_factory):
    left_wall = Line(Vector(0, 0), Vector(0, 50))
    interaction = interaction_factory(lines=[left_wall])
    ball = Ball(Vector(1, 10), Vector(-2, 0), 5)
...
def test_gravity_moves_smaller_ball_towards_larger(interaction_factory):
    small = Ball(Vector(0, 0), Vector(0, 0), 5)
    large = Ball(Vector(10, 0), Vector(0, 0), 15)
    interaction = interaction_factory()
...
def test_enemy_split_creates_new_enemy_and_adjusts_radius(interaction_factory):
    random.seed(0)
    enemy = Enemy(Vector(50, 50), Vector(1, 0), 20)
    interaction = interaction_factory(enemies=[enemy])

    interaction.enemy_split()

    assert len(interaction.enemy) == 2
    radii = sorted([e.radius for e in interaction.enemy])
    assert radii == [9, 11]
    new_enemy = [e for e in interaction.enemy if e.radius == 11][0]
...
def test_add_power_up_respects_limit(interaction_factory):
    interaction = interaction_factory()
    interaction.power_ups = [
        Ball(Vector(0, 0), Vector(0, 0), 1),
        Ball(Vector(0, 0), Vector(0, 0), 1),
        Ball(Vector(0, 0), Vector(0, 0), 1),
        Ball(Vector(0, 0), Vector(0, 0), 1),
        Ball(Vector(0, 0), Vector(0, 0), 1),
    ...
    position = interaction.power_ups[0].position
    assert 5 <= position.x <= 790
    assert 5 <= position.y <= 490


def test_countdown_and_reset_power_up(interaction_factory):
    interaction = interaction_factory(time_limit=3)
...
def test_player_controls_ejects_mass_and_limits_velocity(interaction_factory):
    player = Player(Vector(40, 40), Vector(0, 0), 20)
    keyboard = Keyboard()
    keyboard.right = True
...
def test_game_finish_stops_timers_on_win(interaction_factory, simplegui_stub, frame_stub):
    interaction = interaction_factory()
