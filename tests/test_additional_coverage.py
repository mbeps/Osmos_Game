...
from tests.conftest import DummyCanvas, DummyFrame, DummyTimer

InteractionFactory = Callable[..., Interaction]

def test_dummy_timer_and_canvas_helpers() -> None:
    calls: list[str] = []
    timer: DummyTimer = DummyTimer(50, lambda: calls.append("tick"))

    timer.trigger()

    assert calls == ["tick"]
    canvas: DummyCanvas = DummyCanvas()
    canvas.draw_line((0, 0), (1, 1), 2, "c")
    assert canvas.calls[-1][0] == "line"


def test_line_draw_invokes_canvas(dummy_canvas: DummyCanvas) -> None:
    line: Line = Line(Vector(0, 0), Vector(1, 0))
...
def test_draw_covers_all_entities(
    interaction_factory: InteractionFactory, dummy_canvas: DummyCanvas
) -> None:
...
def test_player_controls_with_speed_power_up(
    interaction_factory: InteractionFactory,
) -> None:
    player: Player = Player(Vector(10, 10), Vector(0, 0), 20)
    keyboard: Keyboard = Keyboard()
    keyboard.right = True
    keyboard.left = True
    keyboard.up = True
...
def test_update_enemy_handles_collisions(
    interaction_factory: InteractionFactory,
) -> None:
    player: Player = Player(Vector(0, 0), Vector(0, 0), 1)
    enemy1: Enemy = Enemy(Vector(0, 0), Vector(0, 0), 5)
    enemy2: Enemy = Enemy(Vector(0, 0), Vector(0, 0), 4)
    mass: Mass = Mass(Vector(0, 0), Vector(0, 0), 1)
    interaction: Interaction = interaction_factory(player=player, enemies=[enemy1, enemy2])
...
def test_enemy_split_handles_vertical_velocity(
    interaction_factory: InteractionFactory,
) -> None:
    random.seed(1)
    enemy: Enemy = Enemy(Vector(50, 50), Vector(0, 1), 20)
    interaction: Interaction = interaction_factory(enemies=[enemy])

    interaction.enemy_split()

    assert len(interaction.enemy) == 2
    new_enemy: Enemy = [e for e in interaction.enemy if e is not enemy][0]
    assert new_enemy.velocity.x == pytest.approx(1)


def test_update_mass_engulfs_on_collision(
    interaction_factory: InteractionFactory,
) -> None:
    player: Player = Player(Vector(0, 0), Vector(0, 0), 10)
    mass: Mass = Mass(Vector(0, 0), Vector(0, 0), 1)
    interaction: Interaction = interaction_factory(player=player)
    interaction.mass = [mass]
...
def test_update_power_ups_grants_speed(
    interaction_factory: InteractionFactory,
) -> None:
    player: Player = Player(Vector(0, 0), Vector(0, 0), 10)
    power: Power_Up = Power_Up(Vector(0, 0))
    interaction: Interaction = interaction_factory(player=player)
...
def test_bounce_without_collision_resets_flag(
    interaction_factory: InteractionFactory,
) -> None:
    interaction: Interaction = interaction_factory()
    ball: Ball = Ball(Vector(50, 40), Vector(0, 0), 1)
...
def test_game_finish_handles_player_death(
    interaction_factory: InteractionFactory,
    simplegui_stub: types.SimpleNamespace,
    frame_stub: DummyFrame,
) -> None:
    interaction: Interaction = interaction_factory(frame=frame_stub)
...
def test_game_finish_handles_timeout(
    interaction_factory: InteractionFactory,
    simplegui_stub: types.SimpleNamespace,
    frame_stub: DummyFrame,
) -> None:
    interaction: Interaction = interaction_factory(time_limit=0, frame=frame_stub)
...
def test_game_finish_handles_exit_key(
    interaction_factory: InteractionFactory,
    simplegui_stub: types.SimpleNamespace,
    frame_stub: DummyFrame,
) -> None:
    keyboard: Keyboard = Keyboard()
    keyboard.e = True
    interaction: Interaction = interaction_factory(keyboard=keyboard, frame=frame_stub, time_limit=5)
